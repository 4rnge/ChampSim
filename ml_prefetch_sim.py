...
def run_command():
    ...
    if not args.no_base:
        for name, binary in zip(baseline_names, baseline_binaries):
            ...
            if seed is not None:
                cmd = '{binary} -prefetch_warmup_instructions {warm} -simulation_instructions {sim} -seed {seed} -traces {trace} > {results}/{base_trace}-{base_binary}.txt 2>&1'.format(
                ...
                cmd = '{binary} -prefetch_warmup_instructions {warm} -simulation_instructions {sim} -traces {trace} > {results}/{base_trace}-{base_binary}.txt 2>&1'.format(
                    binary=binary, warm=args.num_prefetch_warmup_instructions, sim=args.num_instructions,
    ...
    if args.prefetch is not None:
        ...
        if seed is not None:
            cmd = '<{prefetch} {binary} -prefetch_warmup_instructions {warm} -simulation_instructions {sim} -seed {seed} -traces {trace} > {results}/{base_trace}-{base_binary}.txt 2>&1'.format(
            ...
            cmd = '<{prefetch} {binary} -prefetch_warmup_instructions {warm} -simulation_instructions {sim} -traces {trace} > {results}/{base_trace}-{base_binary}.txt 2>&1'.format(
                prefetch=args.prefetch, binary=default_prefetcher_binary, warm=args.num_prefetch_warmup_instructions, sim=args.num_instructions,
                trace=execution_trace, results=args.results_dir, base_trace=os.path.basename(execution_trace),
